...
static int headergen_populate_info(struct headergen_parser *parser, const char *data)
{
    switch (parser->current_level)
    {
        case HEADERGEN_LEVEL_DEVICE:

        switch (parser->current_member)
        {
...
void headergen_parser_init(struct headergen_parser *parser, struct lcrs_node *root)
{
    parser->current_level = HEADERGEN_LEVEL_DEVICE;
    parser->current_member = HEADERGEN_MEMBER_NAME;
...
int headergen_process_token(struct headergen_parser *parser, headergen_token_t token)
{
    ...
    case HEADERGEN_BLOCK_START:
        ...
        if (parser->current_level == HEADERGEN_LEVEL_DEVICE) 
        {
        ...
        else if (parser->current_level == HEADERGEN_LEVEL_REGISTER)
        {
        ...
        else if (parser->current_level == HEADERGEN_LEVEL_FIELD)
        {
        ...
        else if (parser->current_level == HEADERGEN_LEVEL_OPTION)
        {
    ...
    case HEADERGEN_KEY:
        parser->current_member = headergen_parse_key(token.data);
        break;
...
static void headergen_write_traverse(FILE *fh, struct headergen_parser *parser, headergen_node_t *node, int level)
{
